...
class KanjiApp {
  ...
  bindEvents() {
    ...
    // 「もどる」ボタン押下時：セッション内に変更があれば裏でGASへ同期
  ...
  handleRestartAll() {
    ...
    // もう一度書き直す際は、初回試行ではないため true を維持
      ...
      if (isAllSuccess) {
        ...
        setTimeout(async () => {
          if (isFinalQuestion) {
            ...
            // 通常モード時：セットクリア記録 ＋ charStats をGASへ保存
            if (!this.isChallengeMode) {
              ...
              if (currentUser) {
                const progress = Storage.getProgress();
                // ★ backend.gs の期待通り charStats を第4引数として送信
