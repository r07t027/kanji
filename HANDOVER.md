```markdown
# 📖 漢字練習Webアプリ「かんトレ」システム設計・保守・データ定義完全ガイド（HANDOVER v11）

## 1. アプリケーション概要と基本方針

本プロジェクトは、小学校児童向けのタブレット（Chromebook / iPad 等）およびPC環境に最適化された **漢字手書き練習Webアプリケーション「かんトレ」**です[cite: 1]。
...
1. **MASTER スプレッドシート**（ID: `15MNUjS1D9pk4i6miH6dQX7aWpAnyTMXTa_DhpkziaNM`）[cite: 1]
   ...
   * **`progress` シート**: 進捗サマリー（1児童1行）[cite: 1]
     * A列: `userId`
     * B列: `clearedSets`（連想配列 `{"1学期_01": "ISO日時", ...}`）[cite: 1]
...
3. **GAS Web API (`backend.gs`) アクション仕様**[cite: 1]
   ...
   * `saveProgressAndLog`: 単元クリア・チャレンジクリア・特訓克服時の進捗更新（MASTER B/C列マージ）＋詳細ログ追記（LOGシート）[cite: 1]。
   * `updateProgress`: 「もどる」ボタン押下時等のバックグラウンド軽量進捗同期（`keepalive: true`）[cite: 1]。
...
├── css/
│   ├── base.css                  # リセット・共通レイアウト・スプラッシュ画面スタイル（高さ固定絶対配置）
...
└── js/
    ├── main.js                   # アプリ統括・スプラッシュ制御・起動時ポップアップチェーン・「はじめる」待機
    ...
    ├── drill.js                  # 特訓モード（3回合格・6文字バッチ・消滅演出・LOG追記連携）
    ├── challenge.js              # 「かきまるからのちょうせん！」5問自動生成ロジック
    ├── messages.js               # アプリ定数・ユーティリティ・かきまるセリフ集
    ├── audio.js                  # Web Audio API プリロード・完全ミュート制御（setAudioMuted）・安全アンロック
    └── logger.js                 # GAS Web API 通信（データ型保護ヘルパー・prefetch / save / update等）
...
### ② 音のON/OFF（完全ミュート）機能とスプレッドシートG列同期（v9）
...
* **未ログイン時の初期化保証**: `init()` 完了時に全画面ローディング（`#app-loading-screen`）を確実に非表示化。未ログイン状態での日次モーダル（特訓・挑戦状）の誤爆発火を抑止し、ログインモーダルを最前面に正しく表示する構造に分離。


* **ローカル名簿フォールバック**: オフライン時やスプレッドシート通信エラー時でも `data/users.json` から名簿を構築し、画面停止を防止。
* **かきまる挑戦状ヘッダーアイコン常時表示**: 挑戦権がある当日は、挑戦状モーダルの開閉状態や特訓モーダルからの画面遷移にかかわらず、ヘッダー右上の挑戦アイコン（`#btn-header-challenge`）を常時表示（`display: flex`）として維持。



### ⑤ クラウド同期堅牢化・起動時Autoplay対策・UIシフト完全防止（v11・今回）

* **スプレッドシート破壊防止（データ型正規化ガード）**:
* `logger.js` 内に `normalizeClearedSetsPayload` を新設。配列・オブジェクトが混在した不正なデータが送信され、B列・C列が破損してスプレッドシートが開けなくなる障害を根本防止。


* **LOGシート追記漏れ（ログ消失）の復元**:
* 「かきまるチャレンジ」全問クリア時、および「苦手漢字特訓」3回正解克服時に `saveProgressAndLogs` を呼び出すよう修正。未記録状態を完全に解消。


* **単元プレイ直後の特訓モーダル誤爆防止**:
* `checkDailyPopups` の発火タイミングを「アプリ起動時（ログイン時）」のみに厳格限定。単元クリア直後や練習中の中断時に、新規登録された苦手漢字の特訓モーダルが即座に割り込む挙動を排除（次回ログイン時まで待機）。


* **「はじめる！」ボタン新設による Web Audio API アンロック**:
* キャッシュ保持時の自動ログインにおいて、データ準備完了時にスピナーを隠して「はじめる！」ボタンを表示。
* ユーザーの初回タップを検知してからスプラッシュ画面を解除することで、ブラウザのAutoplay（自動音声再生）制限を自然にクリアし、初回ポップアップ（特訓・挑戦状）の効果音を確実に鳴動。


* **スプラッシュ画面のレイアウトシフト（ロゴ位置ズレ）完全防止**:
* `.loading-action-area` の外寸を横幅280px × 高さ88pxに完全固定し、スピナー枠と「はじめる！」ボタンを `position: absolute` で中央に重ねて配置。切り替え時の数ピクセルの下振れ・ガタつきを0px化。
